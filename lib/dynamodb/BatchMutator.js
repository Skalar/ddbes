...
class BatchMutator {
  ...
  constructor({
    ...
    Object.assign(this, {queueSize, maxItemsPerRequest, AWS, tableName})
  }
  ...
  async delete(commits) {
    for await (const commit of this.asIterable(commits)) {
      const {a, k} = serializeCommit(commit)
  ...
  async put(commits) {
    for await (const commit of this.asIterable(commits)) {
      await this.addToQueue({PutRequest: {Item: serializeCommit(commit)}})
    }
  }

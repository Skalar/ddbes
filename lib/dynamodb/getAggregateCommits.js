...
async function getAggregateCommits(
  {aggregateId, minVersion = 1, maxVersion, maxTime, upcasters = {}} = {},
  resultHandlerFn
) {
  ...
  do {
    ...
    if (maxTime) {
      ...
      await resultHandlerFn(
        filteredCommits.map(commit => deserializeCommit(commit, {upcasters})),
        queryResult
      )
    } else {
      await resultHandlerFn(
        queryResult.Items.map(commit => deserializeCommit(commit, {upcasters})),
        queryResult
      )
    }

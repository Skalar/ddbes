...
function eventIsSubscribedTo(event, filterSets) {
  for (const filterSet of filterSets) {
    for (const filterKey of Object.keys(filterSet)) {
      if (Array.isArray(filterSet[filterKey])) {
        if (!filterSet[filterKey].includes(event[filterKey])) continue
      } else if (
        typeof filterSet[filterKey] === 'object' &&
        filterSet[filterKey].regexp
      ) {
        if (
          typeof event[filterKey] !== 'string' ||
          !event[filterKey].match(filterSet[filterKey].regexp)
        ) {
          continue
        }
      } else if (typeof filterSet[filterKey] === 'string') {
        if (event[filterKey] !== filterSet[filterKey]) continue
      } else {
        continue
      }
    }
      ...
      for (const {events, ...commitRest} of commits) {
        for (const event of events) {
          ...
          for (const client of this.server.clients) {
            if (eventIsSubscribedTo(decoratedEvent, client.filterSets)) {
              matches++

...
class Aggregate {
  ...
  // A map of {commandName: functionThatReturnsPromise}
  static commands = {}
  ...
  constructor(constructorConfig = {}) {
    ...
    // Set up direct access to commands on aggregate instance, for convenience
    for (const commandName of Object.keys(this.constructor.commands)) {
      this[commandName] = (...args) => this.execute(commandName, ...args)
    }
  }
  ...
  async execute(commandName, ...args) {
    const command = this.constructor.commands[commandName]

    if (!command) throw new Error(`Command '${commandName}' is not defined`)

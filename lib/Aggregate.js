...
class Aggregate {
  ...
  async executeCommand(
    ...
    throw new Error(`${attempts} out of date errors. Retries exhausted.`)
  }
  ...
  async hydrate({version: requestedVersion, time: requestedTime} = {}) {
    ...
    const query = () => {
      ...
      if (requestedVersion) {
        this.logger.debug(
          `hydrate() Fetching commits where version is > ${this
            .version} and <= ${requestedVersion} from store`
        )
      ...
      this.logger.debug(
        `hydrate() Fetching commits for version > ${this.version} from store`
      )

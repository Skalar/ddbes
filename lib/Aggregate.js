    ...
    return await retryPromiseWithJitteredBackoff(
      async attempts => {
        ...
        const commitData = {
          aggregateType,
          aggregateKey,
          aggregateVersion: headCommit ? headCommit.aggregateVersion + 1 : 1,
          events,
        }

        const commitRecord = await dynamodb.serializeCommit(commitData)
        ...
        return await dynamodb.deserializeCommit(commitRecord)
      },
      {
        initialDelay: 5,
        maxAttempts: 10,
  ...
  async commit(eventOrEvents, {retry = false} = {}) {
    ...
    const perform = async () => {
      try {
        ...
        this.processCommit(await dynamodb.deserializeCommit(commitRecord))
  ...
  async hydrate({
    ...
    if (this.config.snapshots) {
      ...
      if (snapshot) {
        ...
        const {
          state,
          aggregateVersion: snapshotAggregateVersion,
          upcastersChecksum,
          headCommitTimestamp,
        } = snapshot
        if (upcastersChecksum !== this.constructor.upcastersChecksum) {
          ...
          shouldUseSnapshot = requestedVersion >= snapshotAggregateVersion
        } else if (requestedTime) {
          shouldUseSnapshot = requestedTimeString >= headCommitTimestamp
        } else if (snapshotAggregateVersion > this.version) {
          shouldUseSnapshot = true
        }

        if (shouldUseSnapshot) {
          this._state = this.constructor.jsToState(state)
          this.version = snapshotAggregateVersion
          snapshotVersionLoaded = snapshotAggregateVersion
        }
      }
    }

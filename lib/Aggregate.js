...
class Aggregate {
  ...
  static logger = defaultConfig.logger
  ...
  async commit(eventOrEvents, {retry = false} = {}) {
    ...
    const perform = async () => {
      try {
        ...
        this.processCommits([dynamodb.deserializeCommit(commitRecord)])
  ...
  async hydrate({version: requestedVersion, time: requestedTime} = {}) {
    ...
    if (this.config.snapshots && !requestedVersion && !requestedTime) {
      ...
      if (snapshot) {
        const {state, version, upcastersChecksum} = snapshot
  ...
  processCommits(commits = []) {
    ...
    this.headCommitId = commits[commits.length - 1].commitId
  }
  ...
  static get upcastersChecksum() {
    return Object.keys(this.upcasters).length
      ? createHash('md5')
          .update(JSON.stringify(this.upcasters))
          .digest('base64')
      : undefined
  }
  ...
  static async eachInstance(
    ...
    if (aggregateHandlerQueue.length()) {
